 ...
 * 
 * Плюсы:
 *  + Её можно распараллелить.
 * 
 * Минусы:
 *  - Поскольку в худшем случае быстрая сортировка работает за время O(N^2),
 *    заимствовать её из готовых библиотек небезопасно с криптографической точки зрения.
 *    Если алгоритм использует простую стратегию разделения элементов, взломщик может
 *    спровоцировать отказ, сформировав массив, который содержит элементы в порядке, 
 *    дающем наихудшую производительность, и заставить вашу программу пройти по нему.
 *    
 *    Лучше использовать случайные стратегии выбора разделяющего элемента.
 */
void QickSort(Array& values, size_t start_pos, size_t end_pos) {
    /// Если в массиве не больше одного элемента, значит, он отсортирован.
